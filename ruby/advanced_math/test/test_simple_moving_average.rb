...
class SimpleMovingAverageTest < Test::Unit::TestCase
  ...
  def test_sma_initialize_nil
    assert_raise(ArgumentError) { Math::SimpleMovingAverage.new(nil); } 
  end

  # Verify the constructor raises an ArgumentError if the "range" is invalid
  def test_sma_initialize_negative
    assert_raise(ArgumentError) { Math::SimpleMovingAverage.new(-1); }
  end

  # Verify the constructor raises an ArgumentError if the "range" is invalid
  def test_sma_initialize_zero
    assert_raise(ArgumentError) { Math::SimpleMovingAverage.new(0); }
  end

  # Verify the constructor doesn't raise an exception if the value is positive
  def test_sma_initialize_positive
    (1...1000).each do |i| 
      assert_nothing_raised(ArgumentError) { Math::SimpleMovingAverage.new(i); }
    end
  end

  # Verify "add" raises an ArgumentError if the "value" is invalid
  def test_sma_add_nil
    assert_raise(ArgumentError) { Math::SimpleMovingAverage.new(1).add(nil); }
  end

  # Verify SMA of 1, always returns the same value
  def test_sma_add_1
    sma = Math::SimpleMovingAverage.new(1);
    (1...1000).each do |i| 
  ...
  def test_sma_add_2
    sma = Math::SimpleMovingAverage.new(2);
